...
export class InstallJQuery {
	...
	async install(framework: FramewordProvider) {
		try {
			return this._terminalRepository.run(framework.addJQuery(), framework.getProject().id, this._loggerRepository);
		} catch (error) {
			console.error(error);
		}
		return;
	...
	importCss(framework: FramewordProvider) {
		framework.importJQuery(this._fileManagerRepository);
	}
}